...
@Composable
internal fun AsyncTechniqueView(
    modifier: Modifier = Modifier,
    isViewVisible: Boolean,
    isAsyncTechniquePagerVisible: Boolean,
    asyncMethods: List<AsyncMethod>,
    ...
    LaunchedEffect(pagerOffsetFractionChanged) {
        if (pagerOffsetFractionChanged) {
            snapshotFlow { pagerState.currentPage }.collectLatest { page ->
                ...
                onEvent(CompressorScreenViewIntent.OnAsyncMethodPagerScroll(page))
    ...
    AnimatedVisibility(
        ...
        Column(
            ...
            Text(
                modifier = Modifier
                    .clickable {
                        onEvent(CompressorScreenViewIntent.OnChangeAsyncVisibility)
                    },
                style = LocalPETypography.current.titleLarge,
                text = stringResource(R.string.compressor_async_technique),
            )

            AnimatedVisibility(
                visible = isAsyncTechniquePagerVisible,
                enter = expandVertically(
                    animationSpec = tween(durationMillis = 200),
                    clip = false
                ) + fadeIn(),
